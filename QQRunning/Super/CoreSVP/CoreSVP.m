...
{
    [self showSVPWithType:CoreSVPTypeCenterMsg Msg:message duration:kDuration allowEdit:NO beginBlock:nil completeBlock:nil];
}
+(void)showMessageInBottomWithMessage:(NSString *)message
{
    [self showSVPWithType:CoreSVPTypeBottomMsg Msg:message duration:kDuration allowEdit:NO beginBlock:nil completeBlock:nil];
}
+(void)startDownloadDataWithMessage:(NSString *)message
{
    [self hudSetting];
    if (!message || [message trimString].length == 0) {
        message = @"加载中...";
    }
    [self showSVPWithType:CoreSVPTypeLoadingInterface Msg:message duration:kDuration allowEdit:NO beginBlock:nil completeBlock:nil];
}
+(void)showMessage:(NSString *)message WithCode:(NSString *)code
{
    [self hudSetting];
    [SVProgressHUD setDuration:kDuration];
    
    if ([code isEqualToString:@"0"]) {
        [self showSVPWithType:CoreSVPTypeError Msg:message duration:kDuration allowEdit:NO beginBlock:nil completeBlock:nil];
    }
    if ([code isEqualToString:@"1"]) {
        [self showSVPWithType:CoreSVPTypeSuccess Msg:message duration:kDuration allowEdit:NO beginBlock:nil completeBlock:nil];
    }
}
...
+(void)stopDownloadDataWithMessage:(NSString *)message WithCode:(NSString *)code FailBlock:(void(^)())failBlock SuceedBlock:(void(^)())suceedBlok
{
    [self hudSetting];
    if ([code isEqualToString:@"0"]) {
        [self showSVPWithType:CoreSVPTypeError Msg:message duration:kDuration allowEdit:NO beginBlock:nil completeBlock:^{
            if (failBlock) {
                failBlock();
            }
    ...
    if ([code isEqualToString:@"1"]) {
        [self showSVPWithType:CoreSVPTypeSuccess Msg:message duration:kDuration allowEdit:NO beginBlock:nil completeBlock:^{
            if (suceedBlok) {
                suceedBlok();
            }
