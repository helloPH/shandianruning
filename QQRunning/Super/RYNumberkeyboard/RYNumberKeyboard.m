...
- (IBAction)keyboardViewAction:(UIButton *)sender
{
    ...
    switch (tag)
    {
        ...
        case 1011:
        {
            if (([self trimStringwithString:self.textInput.text].length+1) == [self.maxLength integerValue]){
                // 身份证X
        ...
        default:
        {
            ...
            if ([self trimStringwithString:self.textInput.text].length != [self.maxLength integerValue]){
                
                if (self.textInput.text.length != 0) {
                    if(self.interval && [self.interval integerValue] != 0 && ([self trimStringwithString:self.textInput.text].length) % ([self.interval integerValue]) == 0){
...
- (NSString *)trimStringwithString:(NSString *)str
{
